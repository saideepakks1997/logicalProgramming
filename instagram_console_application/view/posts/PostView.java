...
public class PostView {
		...
		while(loop) {
			...
			if(listOfTags.size() > 0) {
				...
				try {
					...
					commonView.displayMessege("Still you want to remove tag \n"
							+ "Enter 1->Yes\n"
							+ "2->No");
					int option = commonView.getOption();
					if(option == 2) {
						loop = false;
					}
				}
				catch (Exception e) {
					commonView.displayMessege("Enter correct number");
				}
			}
			else {
				commonView.displayMessege("No tags available");
				loop = false;
			}
		}
	...
	private Post askAndSelectPost() {
		...
		while(loop) {
			...
			try {
				...
				loop = false;
				commonView.displayMessege("Please enter correct post number");
			}
		}
	...
	private void viewSelfPosts() {
		...
		if(myPosts.size() == 0)
			commonView.displayMessege("No Posts Found");
	}
...
//		this.profile_owner = profile_owner;
	...
	private void createPost() {
		...
		commonView.displayMessege("Post Created successfully");
	}
		...
		while(loop) {
			...
			if(users.size() >0) {
				...
				try {
					...
					if(!isTagged)
						commonView.displayMessege(taggedUser+ " already tagged");
				}
				catch(Exception e) {
					commonView.displayMessege("Enter the correct option");
				}
			}
			else {
				commonView.displayMessege("No users found for the searched name");
			}
	...
	private void viewPosts(InstaUser profile_owner) {
		...
		if(posts == null)
			...
			int i=1;
			for(Post post:posts)
				commonView.displayPosts(post, i++);
	...
	private void getPostNumberFromUserForLikeComment(List<Post> posts, InstaUser profile_owner) {
		commonView.displayMessege("Select post number\n"
					+ "Enter (-1)-->Previous menu");
			int postNo = commonView.getOption();
			if(postNo != -1) {
				try {
					...
					commonView.displayMessege("Enter correct post number");
				}
			}
		}
